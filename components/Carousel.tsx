...
const Carousel = ({topPodcast}: CarouselProps) => {
  ...
  const slides = topPodcast && topPodcast?.filter((item: any) => item.totalPodcasts > 0)
                ...
                slides.slice(0,5).map((item) => (
                    <figure key={item._id} className='carousel_box' onClick={() => router.push(`/podcast/${item.podcast[0]?.podcastId}`)}>
                        ...
                        <div className='glassmorphism-black relative z-10 flex flex-col rounded-b-xl'>
                            ...
                            <p className='text-12 font-normal rounded-xl border-none'>{item.name}</p>
                        </div>
                    </figure>
                ))
            }
